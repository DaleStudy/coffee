...
function mockChannelType(type: number) {
	return (urlStr: string) => {
		if (urlStr.endsWith(`/channels/${TEST_CHANNEL_ID}`)) {
...
describe("createGroupThreads", () => {
	describe("텍스트 채널", () => {
		const checkChannelType = mockChannelType(0);
		...
		test("각 그룹마다 쓰레드를 생성하고 초기 메시지를 보낸다", async () => {
			...
			globalThis.fetch = mock(
				async (url: string | URL | Request, options: RequestInit) => {
					...
					const channelRes = checkChannelType(urlStr);
	...
	describe("포럼 채널", () => {
		const checkChannelType = mockChannelType(15);

		test("각 그룹마다 메시지를 포함한 포스트를 생성한다", async () => {
			const calls: { url: string; body: string }[] = [];
			...
			// 포럼 채널: 그룹당 1회 API 호출 (메시지 포함)
			expect(calls.length).toBe(2);

			// 첫 번째 포스트
			expect(calls[0].url).toContain(`/channels/${TEST_CHANNEL_ID}/threads`);
			const post1Body = JSON.parse(calls[0].body);
			expect(post1Body.name).toBe("☕ 커피챗 1조");
			expect(post1Body.type).toBeUndefined();
			expect(post1Body.auto_archive_duration).toBe(10080);
			expect(post1Body.message.content).toContain("<@1>");
			expect(post1Body.message.content).toContain("<@2>");

			// 두 번째 포스트
			const post2Body = JSON.parse(calls[1].body);
			expect(post2Body.name).toBe("☕ 커피챗 2조");
			expect(post2Body.message.content).toContain("<@3>");
			expect(post2Body.message.content).toContain("<@4>");
		});

		test("하나의 포스트 생성이 실패해도 나머지는 계속 진행된다", async () => {
			const threadCalls: string[] = [];
		...
		test("포스트 이름이 100자를 초과하면 잘린다", async () => {
			let capturedBody: string | undefined;
